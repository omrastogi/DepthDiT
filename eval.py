...
if "__main__" == __name__:
    ...
    for data in tqdm(dataloader, desc="Evaluating"):
        ...
        valid_mask_image = valid_mask_ts.cpu().numpy() * 255  # Convert to a binary image (0 or 255)
        valid_mask_image = valid_mask_image.astype(np.uint8)  # Ensure the image is in uint8 format
        valid_mask_filename = os.path.join(f"valid_mask.png")
        from PIL import Image
        Image.fromarray(valid_mask_image).save(valid_mask_filename)

        depth_raw_image = (depth_raw * 255 / dataset.max_depth).astype(np.uint8)  # Normalize and convert to uint8
        depth_raw_filename = os.path.join("depth_raw.png")
        Image.fromarray(depth_raw_image).save(depth_raw_filename)
        # Load predictions
