...
def log_validation(model, step, device, vae=None):
    ...
    for prompt in validation_prompts:
        ...
        model_kwargs = dict(data_info={'img_hw': hw, 'aspect_ratio': ar}, mask=emb_masks) # [1, 300]

        dpm_solver = DPMS(model.forward_with_dpmsolver,
                          condition=caption_embs, # [1, 300, 4096]
                          uncondition=null_y, #[1, 300, 4096]
                          cfg_scale=4.5,
...
def train():
    ...
    for epoch in range(start_epoch + 1, config.num_epochs + 1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if load_vae_feat: 
                z = batch[0] #[2, 3, 672, 384] given an image_hw --> [1456, 816]
            else:
                with torch.no_grad():
            ...
            if load_t5_feat:
                ...
                with torch.no_grad():
                    ...
                    y = text_encoder(
                        txt_tokens.input_ids, attention_mask=txt_tokens.attention_mask)[0][:, None] #[2, 1, 300, 4096]
                    y_mask = txt_tokens.attention_mask[:, None, None] #[2, 1, 1, 300]
