...
class BaseDepthDataset(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.ASPECT_RATIO = aspect_ratio_map
        self.resize_to_hw = resize_to_hw          
    ...
    def get_aspect_ratio(self):
        if self.ASPECT_RATIO:
            raster = self._get_data_item(0) # pick one data sample
            height, width = raster[0]['rgb_int'].shape[1], raster[0]['rgb_int'].shape[2]
            aspect_ratio = height / width
            closest_ratio = min(self.ASPECT_RATIO.keys(), key=lambda r: abs(float(r) - aspect_ratio))
            return list(map(int, self.ASPECT_RATIO[closest_ratio]))

        else:
            return self.resize_to_hw
    ...
    def __getitem__(self, index):
        try:
            ...
            error_message = (
                ...
                f"Function: {__name__}\n"  # Current function or module name
                f"Line: {traceback.extract_tb(e.__traceback__)[-1][1]}\n"  # Extract the error line
                f"Error: {str(e)}\n"
                f"Traceback:\n{traceback.format_exc()}"  # Full traceback for debugging
            )
            print(error_message)
