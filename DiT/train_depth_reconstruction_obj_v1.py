...
class DepthTrainer:
    ...
    def create_and_initialize_model(self):
        ...
        self.model = DiT_models[self.args.model](input_size=latent_size, num_classes=self.args.num_classes, task_cond=True)
    ...
    def validation(self):
        ...
        for batch in self.val_loader:
            ...
            for rgb in range(rgb_int.shape[0]):
                ...
                depth_pred, depth_colored_img, pred_uncert = pipe.pipe_task_cond(image, task_emb) 
        ...
        # Log to wandb
        if self.rank == 0:
            wandb.log({f"validation_images": wandb_images})
    ...
    def train(self):
        ...
        for train_step in tqdm(range(self.train_step, self.args.iterations), initial=self.train_step, total=self.args.iterations, desc="Training Progress"):
            ...
            rgb_gt = batch["rgb_norm"].to(self.device)
            depth_gt = batch['depth_raw_norm'].to(self.device)
            ...
            with torch.no_grad():
                rgb_latent = self.vae.encode(rgb_gt).latent_dist.sample().mul_(0.18215)
                depth_latent = encode_depth(depth_gt, self.vae)

            y = torch.zeros(self.batch_size, dtype=torch.long).to(self.device)
            timesteps = torch.randint(0, self.diffusion.num_timesteps, (depth_latent.shape[0],), device=self.device)

            # # Preparing for reconstruction objective
            # gt_latent = torch.cat((depth_latent, rgb_latent), dim=0)
            # input_latent = torch.concat((rgb_latent, rgb_latent), dim=0)
            # valid_mask_down = valid_mask_down.repeat(2, 1, 1, 1)
            # y = y.repeat(2)
            # timesteps = timesteps.repeat(2)
            # # Creating Task embedding 
            # task_emb = torch.tensor([[1, 0], [0, 1]]).float().to(self.device)
            # task_emb = torch.cat([torch.sin(task_emb), torch.cos(task_emb)], dim=-1).repeat_interleave(depth_latent.shape[0], 0)
            
            prob_depth = 0.75 # Probability of choosing depth task (e.g., 0.75 for depth, 0.25 for RGB)
            task = "depth" if torch.bernoulli(torch.tensor([prob_depth])).item() == 1 else "rgb"

            input_latent = rgb_latent  # Assuming RGB latent is always used as input

            if task == "rgb":
                # RGB Task
                gt_latent = rgb_latent
                task_vector = torch.tensor([[0, 1]], dtype=torch.float, device=rgb_latent.device)
            else:
                # Depth Task
                gt_latent = depth_latent
                task_vector = torch.tensor([[1, 0]], dtype=torch.float, device=depth_latent.device)

            # Create task embedding
            task_emb = torch.cat([torch.sin(task_vector), torch.cos(task_vector)], dim=-1)
            # Delete the unncessary variables
            del rgb_latent, depth_latent, depth_gt, rgb_gt, task_vector

            self.config.multi_res_noise = None
            ...
            model_kwargs = dict(y=y, input_img=input_latent, task_emb=task_emb)
            loss_dict = self.diffusion.training_losses(self.model, gt_latent, timesteps, model_kwargs, valid_mask_down, noise)
            loss = loss_dict["loss"].mean()
            if task == "depth":
                depth_loss = loss_dict["loss"].mean()
            else:
                rgb_loss = loss_dict["loss"].mean()
...
torchrun --nnodes=1 \
    --nproc_per_node=2 \
    train_depth_reconstruction_obj.py \
    --model "DiT-XL/2" \
    --valid-mask-loss \
